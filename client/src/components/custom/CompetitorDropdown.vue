...
<template>
  <Collapsible v-model:open="isOpen">
    <CollapsibleTrigger as-child :aria-controls="ariaId">
      <div role="button" tabindex="0" :aria-label="`Details for ${result.id}`"
        ...
        <div class="flex-[2] lg:flex-[1.5] min-w-0 text-left">
          <div class="flex flex-row">
            <div class="mx-2 flex flex-col justify-center">
              <ColoredCircle :color />
            </div>
            <CompetitorLink :name="result.name" :id="result.id" :iso2="result.country_iso2" :event="event"
              class="me-1 flex items-center gap-1" />
            <TooltipProvider :delayDuration="250" v-if="result.sample_size < lowDataWarningThreshold">
              <Tooltip>
    ...
    <CollapsibleContent class="space-y-2" :id="ariaId">
      ...
      <div class="flex flex-col items-center px-2 lg:ms-2 lg:flex-row lg:gap-4">
        <div v-for="attemptIdx in eventAttempts[event]" v-bind:key="attemptIdx"
          class="mb-2 flex items-center gap-2 lg:mb-0">
          <span class="whitespace-nowrap">Attempt {{ attemptIdx }}:</span>
          <div class="lg:max-w-24">
            <ResultEntryField v-if="event != '333fm'" v-model="model[attemptIdx - 1]" />
